...
Do **not** ship a sculpt to SL directly — millions of triangles = catastrophic land impact. The sculpt's job is to be the **source of shape and a normal-map bake target**. You will build a low-poly version in [Lesson 5](retopology.md) and bake this detail onto it in [Lesson 7](pbr-materials.md).
