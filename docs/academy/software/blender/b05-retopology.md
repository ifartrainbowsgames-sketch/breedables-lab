# B05 — Retopology (and why it drives SL land impact)

**Learning path:** [Blender Foundations](index.md) · Lesson 5 of 10
**Production stage:** 3 Retopo/LOD · **Prerequisites:** [B04](b04-sculpting.md)
**Feeds track:** [A03 — Organic Retopology](../../tracks/a03-retopology.md)
...
> Add a specific retopo video to the [videos index](../../resources/videos.md#a03-retopology) when you verify one — A03 currently has an open slot.
...
2. **Physics weight** — driven by the physics/collision shape (see [B10](b10-export-to-sl.md)).
...
3. Use the **Poly Build** tool (or extrude edges) to lay quads along the surface, following the same edge-flow rules from [B03](b03-mesh-modeling.md): loops around joints, along the spine and limbs.
...
Tools that speed this up (optional, evaluate against evidence): **[RetopoFlow](../addon-catalog.md#free-community)** for guided quad drawing; **QuadriFlow** (built into Blender) for a fast auto-retopo base you then clean up. Manual retopo remains the primary path for anything that deforms.
...
- Export each as a separate object/file for the uploader ([B10](b10-export-to-sl.md)).
...
[B06 — UV Unwrapping](b06-uv-unwrapping.md)
