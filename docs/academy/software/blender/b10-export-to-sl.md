# B10 — Exporting to Second Life

**Learning path:** [Blender Foundations](index.md) · Lesson 10 of 10
**Production stage:** 9–10 Export / Upload · **Prerequisites:** [B09](b09-animation.md)
**Feeds track:** [A05 — SL Creature Fixture](../../tracks/a05-sl-fixture.md)
...
> SL viewer UI changes often — pin one **current** community upload walkthrough in the [videos index](../../resources/videos.md#a05-sl-fixture-lsl) rather than an old one.
...
| Source | Link |
|--------|------|
| Manual — Collada (.dae) export | [Collada](https://docs.blender.org/manual/en/latest/files/import_export/collada.html) |
| Manual — glTF 2.0 export | [glTF 2.0](https://docs.blender.org/manual/en/latest/addons/import_export/scene_gltf2.html) |
...
| SL — Calculating land impact | [Calculating land impact](https://community.secondlife.com/knowledgebase/english/calculating-land-impact-r273/) |
...
3. **One material per intended SL face** (max 8). Keep the single-tile UVs from [B06](b06-uv-unwrapping.md).
4. **Name objects** clearly per LOD (e.g. `creature_LOD0`…`LOD3`, `creature_PHYS`).

### Build LODs and physics

1. **LODs:** from [B05](b05-retopology.md) you have High/Medium/Low/Lowest. You'll point the uploader at each. Supplying your own beats auto-decimation for both looks and land impact.
...
5. Read the **land impact** and **upload fee (L$)** the uploader shows. If LI is too high, go back to LODs/physics — don't just accept it. (See [B05](b05-retopology.md) for the LI = max(download, physics, server) rule.)
6. Upload, rez, apply your PBR material/maps, and for animated creatures enable **Animesh** and script the animation triggers (that's [A05](../../tracks/a05-sl-fixture.md)).

### Validate in-world (required)

Rez the creature, apply the material from [B07](b07-texture-painting-pbr.md), play the idle/walk from [B09](b09-animation.md), and screenshot with viewer version, region, and date. Record final land impact.
...
You have taken a creature from zero to an SL-ready, rigged, textured, animated upload. Continue with the outcome labs: [A01](../../tracks/a01-organic-pbr.md) → [A05](../../tracks/a05-sl-fixture.md), and browse the [add-on catalog](../addon-catalog.md) and [tutorial arsenal](../../resources/tutorials.md) to go deeper.
