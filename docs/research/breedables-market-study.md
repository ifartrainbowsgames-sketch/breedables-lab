...
Full case-by-case detail — Sion Chickens, Ozimals, Amaretto Ranch Breedables, BioBreeds, KittyCatS, Fennux, Meeroos, Teulu Breedables, and Snuffles — lives on the companion page: **[Breedables case studies](breedables-case-studies.md)**. That page also lists names that came up in research but couldn't be verified as distinct real products with available sourcing (including the "Cheerie Pie" and "WoW Pets" names sometimes mentioned in breedables discussion), and names mentioned only in passing (Fawns, Kreatures, Wild Kajaera, oYo, Foxtrot, Nixsy) that would need a deeper follow-up pass.
