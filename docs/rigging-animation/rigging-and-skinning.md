...
!!! abstract "Lesson 8 of 10 · [Blender foundations](../modeling/blender/index.md)"
    **Production stage** — 7 Rigging  
    **Prerequisites** — [Lesson 7](../texturing/pbr-materials.md)  
...
- Animesh plays animations on the **Second Life avatar skeleton** (the standard SL bones, including collision/Bento bones). A creature rig for Animesh must ultimately map to **SL's named bones** — this is where **[Avastar](../modeling/blender/addon-catalog.md#paid-but-worth-it)** (paid) or a hand-built SL-bone armature earns its place: it provides the correct SL skeleton and export.
- Animesh has a **triangle limit** and counts toward land impact; keep the mesh lean (see [Lesson 5](../modeling/retopology.md)).
- For learning the *mechanics* of rigging and weighting, **Rigify** (free, bundled) is the studio baseline; for the *actual SL export path* you retarget/transfer to the SL skeleton (Avastar or manual) in [Lesson 10](../second-life/export-and-upload.md).
...
- SL path: [Animesh User Guide](https://wiki.secondlife.com/wiki/Animesh_User_Guide) · tools [Avastar](../modeling/blender/addon-catalog.md#paid-but-worth-it) / [Auto-Rig Pro](../modeling/blender/addon-catalog.md#paid-but-worth-it).
