...
def run_daily_wiki(
    ...
    *,
    ...
    if notify_slack and settings is not None:
        from .notify import NotifyUnavailableError, notify_daily_wiki

        try:
            payload["slack_notify"] = notify_daily_wiki(payload, settings=settings)
        except NotifyUnavailableError as exc:
            payload["slack_notify_skipped"] = str(exc)
        except Exception as exc:  # noqa: BLE001 — surface Apprise/network errors in report
            payload["slack_notify"] = {"ok": False, "error": str(exc)}
    return payload
...
def _render_markdown(payload: dict) -> str:
    ...
    lines.append("Learner guide: `docs/academy/index.md`")
